...
class UserToUserRequest extends Mailable implements ShouldQueue
{
    ...
    public function __construct($event_id, $request_type, $title, $desc, $subject, $img, $sender, $response, $email, $receiver, $sender_user)
    {
        ...
        $description = $desc;
        $link = config('app.url') . "/singinwithemail?email=" . $subject . "&pass=" . $pass;
        $link_authenticated = config('app.url') . "/singinwithemail?email=" . $email . "&pass=" . $pass;

        if ($response) {
            $link = config('app.url') . "/singinwithemail?email=" . $email . '&innerpath=' . $event_id . "&request_type=" . $request_type . "&request=" . $response . "&pass=" . $pass;
        }
